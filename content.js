...
(browser || chrome).storage.sync.get(['whitelist', 'blocklist', 'timeRules'], (data) => {
      ...
      (browser || chrome).runtime.sendMessage(
        ...
        (response) => {
          if ((browser || chrome).runtime.lastError) {
            console.error("❌ Runtime error:", (browser || chrome).runtime.lastError);
            reject((browser || chrome).runtime.lastError.message);
            return;
          }
