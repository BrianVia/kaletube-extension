...
saveButton.addEventListener('click', () => {
  ...
  (browser || chrome).storage.sync.set({ geminiApiKey: apiKey }, () => {
    if ((browser || chrome).runtime.lastError) {
      showStatus('Failed to save API key: ' + (browser || chrome).runtime.lastError.message, 'error');
      return;
    }
...
function loadLists() {
  (browser || chrome).storage.sync.get(['whitelist', 'blocklist'], (data) => {
...
function saveWhitelist() {
  (browser || chrome).storage.sync.set({ whitelist }, () => {
    if ((browser || chrome).runtime.lastError) {
      alert('Failed to save whitelist: ' + (browser || chrome).runtime.lastError.message);
      return;
    }
    renderWhitelist();
  });
...
function saveBlocklist() {
  (browser || chrome).storage.sync.set({ blocklist }, () => {
    if ((browser || chrome).runtime.lastError) {
      alert('Failed to save blocklist: ' + (browser || chrome).runtime.lastError.message);
      return;
    }
    renderBlocklist();
  });
...
function loadTimeRules() {
  (browser || chrome).storage.sync.get(['timeRules'], (data) => {
...
saveTimeRules.addEventListener('click', () => {
  ...
  (browser || chrome).storage.sync.set({ timeRules: rules }, () => {
    if ((browser || chrome).runtime.lastError) {
      showTimeStatus('Failed to save time rules: ' + (browser || chrome).runtime.lastError.message, 'error');
      return;
    }
